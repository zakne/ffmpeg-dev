...
static void decode_mode(VP9TileData *td)
{
        ...
        b->seg_id = !s->s.h.segmentation.update_map ? 0 :
               ...
               (s->s.h.segmentation.temporal &&
                vp56_rac_get_prob_branchy(&td->c,
                    s->s.h.segmentation.pred_prob[td->above_segpred_ctx[col] +
    ...
    if (s->s.h.keyframe || s->s.h.intraonly) {
        uint8_t *a = &td->above_mode_ctx[col * 2];
        ...
        if (b->bs <= BS_8x8) {
            if (s->s.h.segmentation.enabled && s->s.h.segmentation.feat[b->seg_id].skip_enabled) {
                ...
                int c = inter_mode_ctx_lut[td->above_mode_ctx[col + off[b->bs]]]
        ...
        if (s->s.h.filtermode == FILTER_SWITCHABLE) {
            ...
            if (have_a && td->above_mode_ctx[col] >= NEARESTMV) {
        ...
        if (b->bs > BS_8x8) {
            int c = inter_mode_ctx_lut[td->above_mode_ctx[col]][td->left_mode_ctx[row7]];
...
#define SET_CTXS(dir, off, n) \
    do { \
        SPLAT_CTX(td->dir##_skip_ctx[off],      b->skip,          n); \
        SPLAT_CTX(s->dir##_txfm_ctx[off],      b->tx,            n); \
        SPLAT_CTX(td->dir##_partition_ctx[off], dir##_ctx[b->bs], n); \
        if (!s->s.h.keyframe && !s->s.h.intraonly) { \
            SPLAT_CTX(s->dir##_intra_ctx[off], b->intra,   n); \
            SPLAT_CTX(s->dir##_comp_ctx[off],  b->comp,    n); \
            SPLAT_CTX(td->dir##_mode_ctx[off],  b->mode[3], n); \
            if (!b->intra) { \
...
#undef SET_CTXS
    ...
    if (!s->s.h.keyframe && !s->s.h.intraonly) {
        if (b->bs > BS_8x8) {
            ...
            AV_COPY32(&td->above_mv_ctx[col * 2 + 0][0], &b->mv[2][0]);
            AV_COPY32(&td->above_mv_ctx[col * 2 + 0][1], &b->mv[2][1]);
            AV_WN32A(&td->above_mv_ctx[col * 2 + 1][0], mv0);
            AV_WN32A(&td->above_mv_ctx[col * 2 + 1][1], mv1);
        } else {
            int n, mv0 = AV_RN32A(&b->mv[3][0]), mv1 = AV_RN32A(&b->mv[3][1]);

            for (n = 0; n < w4 * 2; n++) {
                AV_WN32A(&td->above_mv_ctx[col * 2 + n][0], mv0);
                AV_WN32A(&td->above_mv_ctx[col * 2 + n][1], mv1);
            }
            for (n = 0; n < h4 * 2; n++) {
...
static av_always_inline int decode_coeffs(VP9TileData *td, int is8bitsperpixel)
{
    ...
    uint8_t *a = &td->above_y_nnz_ctx[col * 2];
...
#define DECODE_UV_COEF_LOOP(step, v) \
    ...
    end_y >>= s->ss_v;
    for (pl = 0; pl < 2; pl++) {
        a = &td->above_uv_nnz_ctx[pl][col << !s->ss_h];
    ...
    if (s->pass < 2) {
        ...
        if (!b->skip) {
            ...
            if (!has_coeffs && b->bs <= BS_8x8 && !b->intra) {
                b->skip = 1;
                memset(&td->above_skip_ctx[col], 1, w4);
...
#define SPLAT_ZERO_YUV(dir, var, off, n, dir2) \
    do { \
        SPLAT_ZERO_CTX(td->dir##_y_##var[off * 2], n * 2); \
        if (s->ss_##dir2) { \
            SPLAT_ZERO_CTX(td->dir##_uv_##var[0][off], n); \
            SPLAT_ZERO_CTX(td->dir##_uv_##var[1][off], n); \
        } else { \
            SPLAT_ZERO_CTX(td->dir##_uv_##var[0][off * 2], n * 2); \
            SPLAT_ZERO_CTX(td->dir##_uv_##var[1][off * 2], n * 2); \
        } \
    } while (0)
    ...
    if (s->s.h.filter.level &&
        ...
        if (!s->filter_lut.lim_lut[lvl]) {
            ...
            td->filter_lut.lim_lut[lvl] = limit;
            td->filter_lut.mblim_lut[lvl] = 2 * (lvl + 2) + limit;
        }
    }
