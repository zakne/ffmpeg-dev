...
static int update_size(VP9TileData *td, int w, int h)
{
...
static int decode_frame_header(VP9TileData *td,
                               const uint8_t *data, int size, int *ref)
{
    ...
    if (s->s.h.filter.sharpness != sharp) // fix this for tile threading
...
{
    VP9Context *s = td->s;
    int c = ((td->above_partition_ctx[col] >> (3 - bl)) & 1) |
                              ...
                              int threadnr)
{
    ...
    uvoff = yoff = 0; // ? fix this
    f = s->s.frames[CUR_FRAME].tf.f;
    ls_y = f->linesize[0];
    ls_uv =f->linesize[1];
    
    memset(td->above_partition_ctx, 0, s->cols);
    memset(td->above_skip_ctx, 0, s->cols);
    if (s->s.h.keyframe || s->s.h.intraonly) {
        memset(td->above_mode_ctx, DC_PRED, s->cols * 2);
    } else {
        memset(td->above_mode_ctx, NEARESTMV, s->cols);
    }
    memset(td->above_y_nnz_ctx, 0, s->sb_cols * 16);
    memset(td->above_uv_nnz_ctx[0], 0, s->sb_cols * 16 >> s->ss_h);
    memset(td->above_uv_nnz_ctx[1], 0, s->sb_cols * 16 >> s->ss_h);
    memset(td->above_segpred_ctx, 0, s->cols);
    
    for (tile_row = td->t_row_start; tile_row < td->t_row_end; tile_row++) {
        set_tile_offset(&td->tile_row_start, &td->tile_row_end,
        ...
        for (row = td->tile_row_start; row < td->tile_row_end;
            ...
            for (tile_col = td->t_col_start; tile_col < td->t_col_end; tile_col++) {
                set_tile_offset(&td->tile_col_start, &td->tile_col_end,
                ...
                if (s->pass != 2) {
                    ...
                    memcpy(&td->c, &td->c_b[tile_col], sizeof(td->c));
                }
                ...
                if (s->pass != 2)
                    memcpy(&td->c_b[tile_col], &td->c, sizeof(td->c));
            }
                            ...
                            int *got_frame, AVPacket *pkt)
{
    ...
    int base, remain, num_jobs;
    int retain_segmap_ref = s->s.frames[REF_FRAME_SEGMAP].segmentation_map &&
                            (!s->s.h.segmentation.enabled || !s->s.h.segmentation.update_map);
    ptrdiff_t yoff, uvoff, ls_y, ls_uv;
    AVFrame *f;

    if ((ret = decode_frame_header(td, data, size, &ref)) < 0) {
        return ret;
    } else if (ret == 0) {
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    
    {
        ...
        base = tile_num/num_jobs;
        remain = tile_num%num_jobs;
        
        int index = 0;
        tile_col = 0;
        tile_row = 0;
        for (int i = 0; i < avctx->thread_count; i++) {
            int count = base+(remain+i)/avctx->thread_count;
            
            td[i]->t_row_start = tile_row;
            td[i]->t_col_start = tile_col;
            for (int j = 0; j < count; index++,j++) {
                tile_col = index%s->s.h.tiling.tile_cols;
                tile_row = (index-tile_col)/s->s.h.tiling.tile_cols;
                ...
                ret = ff_vp56_init_range_decoder(&td[i]->c_b[j], data, tile_size);
                if (ret < 0)
                    return ret;
                if (vp56_rac_get_prob_branchy(&td[i]->c_b[j], 128)) { // marker bit
                    ff_thread_report_progress(&s->s.frames[CUR_FRAME].tf, INT_MAX, 0);
                    return AVERROR_INVALIDDATA;
                }
                data += tile_size;
            ...
            td[i]->t_row_end = tile_row;
            td[i]->t_col_end = tile_col;
        }
        
        avctx->execute2(avctx, decode_tiles, s->td, NULL,
                        num_jobs);
